...
import { normalizeProducts, parseCsvProducts } from './product-schema';
...
function collectCategories(products, override) {
...
async function getVersion() {
    ...
    const meta = await db.collection(META_COLLECTION).findOne({ _id: VERSION_KEY });
    return typeof meta?.value === 'number' ? meta.value : 0;
}
async function bumpVersion() {
    const db = await getDb();
    const result = await db.collection(META_COLLECTION).findOneAndUpdate({ _id: VERSION_KEY }, { $inc: { value: 1 } }, { upsert: true, returnDocument: 'after' });
    return result.value?.value ?? 1;
}
async function replaceProducts(products, categories) {
